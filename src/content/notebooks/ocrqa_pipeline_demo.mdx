...
ocrqa_pipeline(text, language="lb")
```
...
```python
# Using the same German text example as before
ocrqa_pipeline(text, version="1.0.5")
```
...
```python
# Using the same German text example as before
ocrqa_pipeline(text, diagnostics=True)
```
...
```python
# Using the same German text example as before
ocrqa_pipeline(text, model_id=True)
```
...
```python
# Using the same German text example as before
ocrqa_pipeline(text, supported_languages=True)
```
...
```python
short_text_with_unusual_name = "Glebs geht ins Büro."
```

{/* cell:35 cell_type:code */}
```python
# Example 1: Very short sentence
ocrqa_pipeline(short_text_with_unusual_name, diagnostics=True)
```
