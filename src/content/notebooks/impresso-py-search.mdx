---
title: A quick guide to searching with Impresso library
githubUrl: https://github.com/impresso/impresso-datalab-notebooks/blob/main/starter/search_ImpressoAPI.ipynb
authors:
  - impresso-team
seealso:
  - impresso-py-collections
sha: fb6ff6a3d3f3fbeaaaee870c37520bc389d1c93e
date: 2026-04-10T14:45:11Z
googleColabUrl: https://colab.research.google.com/github/impresso/impresso-datalab-notebooks/blob/main/starter/search_ImpressoAPI.ipynb
links:
  - href: https://github.com/impresso/impresso-datalab-notebooks/blob/main/reporting-problems.md
    label: report a problem
  - href: https://impresso.github.io/impresso-py/
    label: Impresso Python Library
  - href: https://impresso-project.ch/app/
    label: Impresso Web App
  - href: https://github.com/impresso/impresso-datalab-notebooks/blob/main/explore-vis/place-entities_map.ipynb
    label: Visualising Place Entities on Maps
  - href: https://impresso-project.ch/datalab/
    label: Impresso Datalab
  - href: https://en.wikipedia.org/wiki/Titanic_(1953_film
    label: Titanic (1953)
  - href: https://dev.impresso-project.ch/public-api/v1/docs/#/search-facets/getSearchFacet
    label: Facets search
  - href: https://dev.impresso-project.ch/public-api/v1/docs/#/search-facets/getSearchFacet
    label: Facets search
  - href: https://github.com/impresso/impresso-datalab-notebooks/blob/main/starter/basics_ImpressoAPI.ipynb
    label: Introduction to the Impresso Python Library
  - href: https://impresso.github.io/impresso-py/
    label: documentation here
  - href: data:image/png;base64,
    label: "![CreditLogo.png"
  - href: https://creativecommons.org/licenses/by/4.0/
    label: CC BY 4.0 License
  - href: https://impresso-project.ch
    label: Impresso - Media Monitoring of the Past
  - href: http://p3.snf.ch/project-173719
    label: CRSII5_173719
  - href: https://data.snf.ch/grants/grant/213585
    label: CRSII5_213585
  - href: https://github.com/impresso/impresso-pyindexation/blob/master/LICENSE
    label: GNU Affero General Public License
excerpt: This notebook provides an overview of the search capabilities of the
  Impresso Python Library.
---

{/* cell:0 cell_type:markdown */}

<a target="_blank" href="https://colab.research.google.com/github/impresso/impresso-datalab-notebooks/blob/main/starter/search_ImpressoAPI.ipynb">
  <img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/>
</a>

If something doesn't work, you can [report a problem](https://github.com/impresso/impresso-datalab-notebooks/blob/main/reporting-problems.md).

{/* cell:1 cell_type:markdown */}
## What is this notebook about?

This notebook serves as a quick guide to searching with [Impresso Python Library](https://impresso.github.io/impresso-py/). By using this, you will be able to find content items in Impresso using a variety of filters. While it is useful to introduce new users to the capabilities of the library, it can also be used as a cheatsheet, to which you can always refer to when you need a rapid solution. 

> Please, note that some news headlines (titles) and transcripts might be shown as **[redacted]** depending on the copyright access your user plan allows.

{/* cell:2 cell_type:markdown */}
## Why is this useful?

While the [Impresso Web App](https://impresso-project.ch/app/) provides users with a powerful user-friendly interface, accessing Impresso data via the API allows for more flexibility in your research pipelines. This means you can, for example, create visualisations beyond those provided by the interface, such as the one described in detail in the Datalab notebook [Visualising Place Entities on Maps](https://github.com/impresso/impresso-datalab-notebooks/blob/main/explore-vis/place-entities_map.ipynb).

Using this Python library, you can, for example, search for contents based on the presence of a certain word in the title. You can also filter results based on whether the content has been published in the newspaper's front page. Other available filters are, for example:

- date of publication
- language of content item
- country of publication
- by newspaper title
- presence of specific entities
- presence on your own collections 



{/* cell:3 cell_type:markdown */}
## What will you learn?

In this notebook, you will learn how to:

- Navigate the Impresso corpus via API
- Use the different search functionalities available on the Impresso Python library.
- Write complex queries (including AND and OR queries)

It is divided into three parts:
- Part 1: Term search
- Part 2: Searching in metadata
- Part 3: Searching with Facets

This notebook will guide you through these core functionalities and help you get familiar with the Impresso library capabilities.

{/* cell:4 cell_type:markdown */}
## Prerequisites

Run the following cell to install the `impresso` python library. You may need to restart the kernel to use updated packages. To do so, on Google Colab, go to *Runtime* and select *Restart session*.

{/* cell:5 cell_type:code */}
```python
from packaging.version import Version

MIN_VERSION = "0.9.15"
try:
    from impresso import version
    assert Version(version) >= Version(MIN_VERSION)
    print(f"✔ impresso {version} is installed and up to date.")
except (ImportError, AssertionError):
    %pip install --upgrade --force-reinstall impresso
```

{/* cell:6 cell_type:markdown */}
## Initialising an Impresso Client

By running the following cell, we create an instance of the Impresso client and authenticate it with the Impresso API.

> The `impresso` variable stores an instance of `ImpressoClient`, which establishes a connection to the API using your authentication token. With this object, you can interact with the API to perform operations such as searching for content items, retrieving entities, and fetching facets.

The following command will prompt you to enter your Impresso token if it has not been authenticated recently (it expires after 8 hours).

To get access to an Impresso API token, go to [Impresso Datalab](https://impresso-project.ch/datalab/) and select *Get API Token* on the menu.

{/* cell:7 cell_type:code */}
```python
from impresso import connect

client = connect()
```

{/* cell:8 cell_type:markdown */}
## Part 1: Term search  

{/* cell:9 cell_type:markdown */}
### Argument `term`

Start by searching for content items containing the keyword 'titanic'.
> In Impresso, a **Content Item** is the smallest unit of editorial content within a newspaper or radio collection. This can be an article (for newspapers) or a radio show or episode (for radio programs). Content items can also vary by type, including articles, advertisements, tables, images, and more. Please note that when a newspaper does not have segmentation (OLR) content items for this title correspond to pages.

**Important note:** on the output, you will find the option 'See this result in the Impresso App'. By clicking on the link, you will be able to visualise the same result you retrieved using this notebook by using the powerful interface of the Impresso Web App. This is part of the Impresso project effort to integrate the workflow of the Web App with the Datalab.

By default, results are limited to 100. You can use the parameter `limit` to extend your results. Eg. (limit = 300). Maximum is 1000.

{/* cell:10 cell_type:code */}
```python
client.search.find(term="titanic")
```

{/* cell:11 cell_type:markdown */}
### Argument `with_text_contents`

Retrieve only content items that contain textual data. 

{/* cell:12 cell_type:code */}
```python
client.search.find(term="titanic", with_text_contents=True)
```

{/* cell:13 cell_type:markdown */}
### Argument `title`
Retrieve only content items that have the keyword 'titanic' in the title (news headline).

{/* cell:14 cell_type:code */}
```python
client.search.find(title="titanic")
```

{/* cell:15 cell_type:markdown */}
### Complex term requests


{/* cell:16 cell_type:markdown */}
### `AND` queries
Find content items that contain more than one term in the news headline.

{/* cell:17 cell_type:code */}
```python
from impresso import AND

client.search.find(title=AND("titanic", "oscars"))
```

{/* cell:18 cell_type:markdown */}
### `OR` queries
Find content items that contain either one or another term.

{/* cell:19 cell_type:code */}
```python
from impresso import OR

client.search.find(title=OR("titanic", "naufrage"))
```

{/* cell:20 cell_type:markdown */}
### Inverted search (everything excluding term A __OR__ term B)

To find all content items containing the word "titanic" in the title that do not mention neither "film" nor "pellicule", you can use `~` before **OR**

{/* cell:21 cell_type:code */}
```python
from impresso import OR

client.search.find(title="titanic", term=~OR("film", "pellicule"))
```

{/* cell:22 cell_type:markdown */}
### Complex combination of terms

The following cell searches all content items with all of the the following conditions:

* mentioning 'titanic' AND '1912'
* also mentioning either 'eisberg' OR 'iceberg'
* and not mentioning 'film' nor 'pellicule'

{/* cell:23 cell_type:code */}
```python
from impresso import AND, OR

client.search.find(term=AND("titanic", "1912") & OR("eisberg", "iceberg") & ~OR("film", "pellicule"))
```

{/* cell:24 cell_type:markdown */}
## Part 2: Searching in metadata 

{/* cell:25 cell_type:markdown */}
### Argument `front_page`

Retrieve content items published on the newspaper's front page only

{/* cell:26 cell_type:code */}
```python
client.search.find(term="titanic", front_page=True)
```

{/* cell:27 cell_type:markdown */}
### Argument `entity_id`

All entities in the Impresso corpus have a specific ID. You can use that ID to retrieve content items where this specific entity is mentioned. 

But where do I find the entity ID? You can simply make a search for the entity using the cell below and the `entity_id` will be shown in the first column.


{/* cell:28 cell_type:code */}
```python
client.entities.find(term="James Cameron")
```

{/* cell:29 cell_type:markdown */}
> PS: There are various entities (10) mentioning James Cameron. We are looking for the film director, so we use the first one in the output. You can check to whom each of the outputs refer by clicking on 'see this result in the Impresso App'. 

Search by entity ID. 

{/* cell:30 cell_type:code */}
```python
client.search.find(entity_id="2-50-James_Cameron")
```

{/* cell:31 cell_type:markdown */}
You can retrieve all content items that mention both the entities James Cameron **AND** Leonardo Dicaprio.

{/* cell:32 cell_type:code */}
```python
client.search.find(entity_id=AND("2-50-James_Cameron", "2-50-Leonardo_DiCaprio"))
```

{/* cell:33 cell_type:markdown */}
Also, find all content items that mention the word 'titanic' and either the entities James Cameron **OR** Barbara Stanwyck (actress in the film [Titanic (1953)](https://en.wikipedia.org/wiki/Titanic_(1953_film)))

{/* cell:34 cell_type:code */}
```python
client.search.find(term="titanic", entity_id=OR("2-50-James_Cameron", "2-50-Barbara_Stanwyck"))
```

{/* cell:35 cell_type:markdown */}
### Argument `newspaper_id`

Retrieve content items that have been published by a specific newspaper with their acronym. 

But how do I find the newspapers' acronyms? 

You can simply use [Facets search](https://dev.impresso-project.ch/public-api/v1/docs/#/search-facets/getSearchFacet) method to retrieve all newspapers that are relevant to your keyword search. The acronyms are shown in **value** column.

{/* cell:36 cell_type:code */}
```python
df_newspapers = client.search.facet("newspaper", term="titanic", limit=100)
df_newspapers.df
```

{/* cell:37 cell_type:markdown */}
In the case below, search for 'titanic' in either EXP (L'Express) or GDL (Gazette de Lausanne)

{/* cell:38 cell_type:code */}
```python
client.search.find(term="titanic", newspaper_id=OR("EXP", "GDL"))
```

{/* cell:39 cell_type:markdown */}
### `DateRange` class

By using `DateRange`, you can delimit a timeframe for your search. In the example below, you will find content items mentioning the word 'titanic', published between 15th April 1912 (date of the accident) and 1st January 1913. 

{/* cell:40 cell_type:code */}
```python
from impresso import DateRange

client.search.find(term="titanic", date_range=DateRange("1912-04-15", "1913-01-01"))
```

{/* cell:41 cell_type:markdown */}
You can also search for content items published outside the date range by using the `~` .

{/* cell:42 cell_type:code */}
```python
from impresso import DateRange

client.search.find(term="titanic", date_range=~DateRange("1912-04-15", "1913-01-01"))
```

{/* cell:43 cell_type:markdown */}
### Argument `language`

Search for the term "titanic" in content items in German or English language.

{/* cell:44 cell_type:code */}
```python
client.search.find(term="titanic", language=OR("de", "en"))
```

{/* cell:45 cell_type:markdown */}
### Argument `collection_id`
You can display all content items you saved in one of your collections.

{/* cell:46 cell_type:code */}
```python
client.search.find(collection_id="ADD_COLLECTION_ID_HERE")
```

{/* cell:47 cell_type:markdown */}
To find the id of one of your collections, you can use the code below:

{/* cell:48 cell_type:code */}
```python
client.search.facet("collection")
```

{/* cell:49 cell_type:markdown */}
### Argument `country`

Find all content items published in either of the two specified countries.

{/* cell:50 cell_type:code */}
```python
client.search.find(term="titanic", country=OR("FR", "CH"))
```

{/* cell:51 cell_type:markdown */}
### Argument `partner_id`

Limit search to content items provided by a specific partner of the Impresso project. E.g.: BNL (Bibliothèque nationale du Luxembourg).

{/* cell:52 cell_type:code */}
```python
client.search.find(term="titanic", partner_id="Migros")
```

{/* cell:53 cell_type:markdown */}
But how do I find the partners' acronyms?

Simply use [Facets search](https://dev.impresso-project.ch/public-api/v1/docs/#/search-facets/getSearchFacet) method to retrieve all partners that are relevant to your keyword search. The acronyms are shown in value column:

{/* cell:54 cell_type:code */}
```python
df_partners = client.search.facet("partner", limit=100)
df_partners.df
```

{/* cell:55 cell_type:markdown */}
## Part 3: Searching with Facets

Facets search is a way to get statistics for your search using Impresso metadata. Facets search methods have the same attributes as the search method.

Some of these Facet search methods have been used above already. Here you will find more:

{/* cell:56 cell_type:markdown */}
### Date range

Get the number of content items that mention 'titanic', published on a particular date.

{/* cell:57 cell_type:code */}
```python
client.search.facet("daterange", term="titanic")
```

{/* cell:58 cell_type:markdown */}
### Year

Get the number of content items that mention 'titanic', published during a particular year.

{/* cell:59 cell_type:code */}
```python
client.search.facet("year", term="titanic")
```

{/* cell:60 cell_type:markdown */}
### Month

Get the number of content items that mention 'titanic', published during a particular month.

> PS: Months are represented by numbers in column 'value'. 1 = January.

{/* cell:61 cell_type:code */}
```python
client.search.facet("month", term="titanic") 
```

{/* cell:62 cell_type:markdown */}
### Content length

Count the number of content items that mention the word 'titanic', group results by content length.

Content length is grouped in bins of 100 words. Each bin is represented by an index. For example:

- Index 0: content items with 0 - 100 words
- Index 1: content items with 101 - 200 words
- Index 2: content items with 201 - 300 words, and so on. 

{/* cell:63 cell_type:markdown */}
> Reminder: In Impresso, a Content Item is the smallest unit of editorial content within a newspaper or radio collection. This can be an article (for newspapers) or a radio show or episode (for radio programs). Content items can also vary by type, including articles, advertisements, tables, images, and more. Please note that when a newspaper does not have segmentation (OLR) content items for this title correspond to pages.

{/* cell:64 cell_type:code */}
```python
client.search.facet("contentLength", term="titanic") 
```

{/* cell:65 cell_type:markdown */}
### Country

Get the number of content items that mention 'titanic', grouped by country of publication.

{/* cell:66 cell_type:code */}
```python
client.search.facet("country", term="titanic")
```

{/* cell:67 cell_type:markdown */}
### Type

Get the number of items that mention 'titanic', grouped by type of item.

Dictionary:

* ad = advertisement
* ar = article
* ob = obtuary 

{/* cell:68 cell_type:code */}
```python
client.search.facet("type", term="titanic")
```

{/* cell:69 cell_type:markdown */}
### Topic

Find topics that the content items mentioning 'titanic' are related to.

{/* cell:70 cell_type:code */}
```python
client.search.facet("topic", term="titanic")
```

{/* cell:71 cell_type:markdown */}
### Collection

Find within your collections, a collection in which a content items containing 'titanic' is stored. This will just work if you have a collection in Impresso. 

{/* cell:72 cell_type:code */}
```python
client.search.facet("collection", term="titanic")
```

{/* cell:73 cell_type:markdown */}
### Newspaper

Find newspapers in which content items mentioning the keyword 'titanic' have been published in.

{/* cell:74 cell_type:code */}
```python
client.search.facet("newspaper", term="titanic")
```

{/* cell:75 cell_type:markdown */}
### Language

Find the languages of content items mentioning the keyword 'titanic'.

{/* cell:76 cell_type:code */}
```python
client.search.facet("language", term="titanic")
```

{/* cell:77 cell_type:markdown */}
Find locations occurring in content items mentioning the keyword 'titanic'.

{/* cell:78 cell_type:code */}
```python
client.search.facet("location", term="titanic")
```

{/* cell:79 cell_type:markdown */}
### Copyright

Get copyright information for content items mentioning 'titanic'. Results are either 'in copyright' (in_cpy) or 'public' (pbl).

{/* cell:80 cell_type:code */}
```python
client.search.facet("copyright", term="titanic")
```

{/* cell:81 cell_type:markdown */}
### Partner

Get Impresso partners that provided content items mentioning 'titanic'.

{/* cell:82 cell_type:code */}
```python
client.search.facet("partner", term="titanic")
```

{/* cell:83 cell_type:markdown */}
## Conclusion
That's it for now! Next, you can explore:

- the [Introduction to the Impresso Python Library](https://github.com/impresso/impresso-datalab-notebooks/blob/main/starter/basics_ImpressoAPI.ipynb) notebook, which demonstrates how to use the Impresso Library further, including managing collections and much more.
- if you want to learn more about the Impresso Python library, you can find its [documentation here](https://impresso.github.io/impresso-py/).

{/* cell:84 cell_type:markdown */}
---
## Project and License info

### Notebook credits [![CreditLogo.png](data:image/png;base64,)](https://credit.niso.org/)
**Writing - Original draft:** Roman Kalyakin. **Conceptualization:** Roman Kalyakin, Maud Ehrmann. **Software:** Roman Kalyakin. **Writing - Review & Editing:** Caio Mello. **Validation:** Sarah Oberbichler, Kirill Veprikov, Cao Vy. **Datalab editorial board:** Caio Mello (Managing), Pauline Conti, Emanuela Boros, Marten Düring, Juri Opitz, Martin Grandjean, Estelle Bunout. **Data curation & Formal analysis:** Maud Ehrmann, Emanuela Boros, Pauline Conti, Simon Clematide, Juri Opitz, Andrianos Michail. **Methodology:** Roman Kalyakin. **Supervision:** Marten Düring, Maud Ehrmann. **Funding aquisition:** Maud Ehrmann, Simon Clematide, Marten Düring, Raphaëlle Ruppen Coutaz.


<a target="_blank" href="https://creativecommons.org/licenses/by/4.0/">
  <img src="https://mirrors.creativecommons.org/presskit/buttons/88x31/png/by.png"  width="100" alt="Open In Colab"/>
</a> 

This notebook is published under [CC BY 4.0 License](https://creativecommons.org/licenses/by/4.0/)

For feedback on this notebook, please send an email to info@impresso-project.ch

### Impresso project

[Impresso - Media Monitoring of the Past](https://impresso-project.ch) is an interdisciplinary research project that aims to develop and consolidate tools for processing and exploring large collections of media archives across modalities, time, languages and national borders. The first project (2017-2021) was funded by the Swiss National Science Foundation under grant No. [CRSII5_173719](http://p3.snf.ch/project-173719) and the second project (2023-2027) by the SNSF under grant No. [CRSII5_213585](https://data.snf.ch/grants/grant/213585) and the Luxembourg National Research Fund under grant No. 17498891.



### License

All Impresso code is published open source under the [GNU Affero General Public License](https://github.com/impresso/impresso-pyindexation/blob/master/LICENSE) v3 or later.


---

<p align="center">
  <img src="https://github.com/impresso/impresso.github.io/blob/master/assets/images/3x1--Yellow-Impresso-Black-on-White--transparent.png?raw=true" width="350" alt="Impresso Project Logo"/>
</p>

